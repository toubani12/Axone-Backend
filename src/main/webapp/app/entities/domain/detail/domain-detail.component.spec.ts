...
describe('Domain Management Detail Component', () => {
  ...
  describe('OnInit', () => {
    it('Should load domain on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', DomainDetailComponent);

      // THEN
      expect(instance.domain()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
